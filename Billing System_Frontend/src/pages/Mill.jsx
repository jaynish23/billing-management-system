...
import { 
  Building, Search, Plus, X, MoreVertical, Edit, Power, History, Download, MapPin, DollarSign, User, Phone
} from 'lucide-react';
...
function Mill() {
  ...
  const handleSubmit = async (e) => {
    ...
    let payload = { 
      ...
      millName: formData.name,
      ownerName: formData.ownerName,
      ownerPhoneNo: formData.ownerPhoneNo,
      location: formData.location,
      districtNo: parseInt(formData.districtNo),
      stateNo: parseInt(formData.stateNo),
      millTaxInfo: formData.millTaxInfo ? parseFloat(formData.millTaxInfo) : null,
      inputLanguage: lang === 'gu' ? 'guj' : lang
    };
    
    try {
      showLoader(editingId ? 'Updating mill record...' : 'Creating new mill record...');
      if (editingId) {
        await api.put(`/mill/${editingId}`, payload);
      } else {
        await api.post('/mill', payload);
      }
      hideLoader();
      if (editingId) {
        showSuccess('Mill Updated Successfully', 'Record updated successfully.');
      } else {
        showSuccess('Mill Created Successfully', 'Record added successfully.');
      }
      setShowForm(false);
      setEditingId(null);
      setFormData({ name: '', ownerName: '', ownerPhoneNo: '', location: '', stateNo: '', districtNo: '', millTaxInfo: '', isActive: true });
      fetchMills();
    } catch (err) {
      console.error(err);
      hideLoader();
      const errMsg = err.response?.data?.message || 'Unable to save mill record.';
      showError(editingId ? 'Mill Update Failed' : 'Mill Creation Failed', errMsg);
    }
  };

  const handleEdit = (item) => {
    setEditingId(item.millNo);
    if (item.stateNo) {
  ...
  const handleToggleActive = async (item) => {
    try {
      showLoader(item.isActive ? 'Deactivating mill...' : 'Activating mill...');
      const payload = { ...item, isActive: !item.isActive };
      await api.put(`/mill/${item.millNo}`, payload);
      hideLoader();
      if (item.isActive) {
        showSuccess('Mill Deactivated', `${item.millName} has been deactivated.`);
      } else {
        showSuccess('Mill Activated', `${item.millName} is now active.`);
      }
      fetchMills();
    } catch (err) {
      console.error(err);
  ...
  const handleAudit = async (recordNo) => {
    try {
      ...
      const res = await api.get(`/audit/${recordNo}?tableName=UserMill`);
      const audits = res.data.filter(a => a.tableName === 'UserMills' || a.tableName === 'UserMill');
  ...
  const handleExport = () => {
    ...
    try {
      showLoader('Exporting mill data...');
      const headers = ["No", "Mill Name", "Owner Name", "Phone", "Location", "Tax Info", "State / District", "Status"];
      const rows = mills.map((item, idx) => [
      ...
      link.setAttribute("download", "mills_export.csv");
  ...
  const activeCount = Math.round(totalCount * 0.82);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl sm:text-3.5xl font-extrabold text-text-primary tracking-tight flex items-center gap-2.5">
            <span>🏢</span>
            <span>{t('mill.title')}</span>
          </h2>
          <p className="text-text-muted text-xs sm:text-sm font-medium mt-1">
            Manage your corporate mills, physical locations, owners, and tax options.
          </p>
        </div>
        <button 
          onClick={() => {
      ...
      >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="glass-card rounded-2xl p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-bold text-text-muted uppercase tracking-wider mb-0.5">Total Mills</p>
          ...
          <div className="glass-card rounded-2xl p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-black text-text-primary">2</p>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-4 border border-border-base flex flex-col md:flex-row md:items-center gap-3">
          <div className="relative flex-1">
            ...
            <input 
              type="text" 
              placeholder="Search mill name, owner or phone..." 
        ...
        <div className="hidden md:block overflow-hidden bg-bg-lighter/30 border border-border-base rounded-3xl shadow-xl">
          <table className="min-w-full divide-y divide-border-base text-left">
            <thead className="bg-bg-base/40">
              <tr>
                <th className="px-6 py-3.5 text-xs font-bold text-text-muted uppercase tracking-wider">{t('mill.no')}</th>
                <th className="px-6 py-3.5 text-xs font-bold text-text-muted uppercase tracking-wider">{t('mill.name')}</th>
                <th className="px-6 py-3.5 text-xs font-bold text-text-muted uppercase tracking-wider">{t('mill.ownerName')}</th>
                <th className="px-6 py-3.5 text-xs font-bold text-text-muted uppercase tracking-wider">{t('mill.phone')}</th>
                <th className="px-6 py-3.5 text-xs font-bold text-text-muted uppercase tracking-wider">{t('mill.location')}</th>
                <th className="px-6 py-3.5 text-xs font-bold text-text-muted tracking-wider text-right uppercase">{t('mill.taxInfo')}</th>
                <th className="px-6 py-3.5 text-xs font-bold text-text-muted uppercase tracking-wider">{t('mill.state')} / {t('mill.district')}</th>
                <th className="px-6 py-3.5 text-xs font-bold text-text-muted uppercase tracking-wider">{t('mill.status')}</th>
                <th className="px-6 py-3.5 text-xs font-bold text-text-muted uppercase tracking-wider w-20">{t('mill.actions')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border-base bg-transparent text-sm">
              {filteredMills.map((item, idx) => (
                <tr key={item.millNo} className="hover:bg-bg-darker/30 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap text-text-muted">{(page - 1) * pageSize + idx + 1}</td>
                  <td className="px-6 py-4 whitespace-nowrap font-bold text-text-primary tracking-tight">{item.millName}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right font-mono text-text-primary">{item.millTaxInfo}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2.5 py-1 inline-flex text-[10px] leading-5 font-bold rounded-full border ${
                      ...
                      {item.isActive ? t('mill.active') : t('mill.inactive')}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <ActionMenu
                      ...
                      onAudit={() => handleAudit(item.millNo)}
                      editLabel={t('mill.edit')}
                      activateLabel={t('mill.activate')}
                      deactivateLabel={t('mill.deactivate')}
                      auditLabel={t('mill.audit')}
                    />
                  </td>
                </tr>
              ))}
              {filteredMills.length === 0 && (
                <tr>
                  <td colSpan="9" className="px-6 py-8 text-center text-sm text-text-muted font-medium">{t('mill.noMills')}</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="grid grid-cols-1 gap-4 md:hidden">
          {filteredMills.map((item) => (
            <div key={item.millNo} className="glass-card rounded-2xl p-4 border border-border-base relative space-y-3">
              <div className="flex justify-between items-start">
                <div>
                  <h4 className="font-bold text-text-primary tracking-tight">{item.millName}</h4>
                ...
                <div className="flex items-center gap-1">
                  <span className={`px-2 py-0.5 text-[9px] leading-5 font-bold rounded-full border ${
                   ...
                   <ActionMenu
                     ...
                     onToggleActive={() => handleToggleActive(item)}
                     onAudit={() => handleAudit(item.millNo)}
                     editLabel={t('mill.edit') || 'Edit'}
                     activateLabel={t('mill.activate') || 'Activate'}
                     deactivateLabel={t('mill.deactivate') || 'Deactivate'}
                     auditLabel={t('mill.audit') || 'Audit'}
                   />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border-base/60 text-xs">
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-text-muted uppercase tracking-wide">{t('mill.phone')}</p>
                ...
                <div className="space-y-1">
                  <p className="text-[10px] font-bold text-text-muted uppercase tracking-wide">{t('mill.state')} / {t('mill.district')}</p>
                  <p className="text-text-secondary font-semibold">{item.stateName} / {item.districtName}</p>
                </div>
                <div className="col-span-2 space-y-1">
                  <p className="text-[10px] font-bold text-text-muted uppercase tracking-wide">{t('mill.location')}</p>
                ...
                <div>
                  <div className="flex items-center justify-between border-b border-border-base pb-4 mb-6">
                    <h3 className="text-lg font-bold text-text-primary flex items-center gap-2">
                      ...
                      <span>{editingId ? t('mill.edit') : t('mill.addMill')}</span>
                    </h3>
                    <button 
                      type="button"
                      onClick={() => setShowForm(false)} 
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        <label className="text-xs font-semibold text-text-secondary">{t('mill.name')} *</label>
                        <input 
                          required 
                          name="name" 
                          value={formData.name} 
                          ...
                          placeholder="E.g. Sunrise Mills Ltd." 
                        />
                      </div>
                      <div>
                        <label className="text-xs font-semibold text-text-secondary">{t('mill.ownerName')} *</label>
                        <input 
                          required 
                          name="ownerName" 
                      ...
                      <div>
                        <label className="text-xs font-semibold text-text-secondary">{t('mill.phone')} *</label>
                        <input 
                          required 
                          name="ownerPhoneNo" 
                    ...
                    <div className="space-y-3 pt-4 border-t border-border-base/60">
                      ...
                      <div>
                        <label className="text-xs font-semibold text-text-secondary">{t('mill.location')}</label>
                        <input 
                          name="location" 
                      ...
                      <div>
                        <label className="text-xs font-semibold text-text-secondary">{t('mill.state')} *</label>
                        <select 
                          required 
                          name="stateNo" 
                      ...
                      <div>
                        <label className="text-xs font-semibold text-text-secondary">{t('mill.district')} *</label>
                        <select 
                          required 
                          name="districtNo" 
                    ...
                    <div className="space-y-3 pt-4 border-t border-border-base/60">
                      ...
                      <div>
                        <label className="text-xs font-semibold text-text-secondary">{t('mill.taxInfo')}</label>
                        <input 
                          type="number" 
                          step="any" 
                          name="millTaxInfo" 
                          value={formData.millTaxInfo} 
                          onChange={handleInputChange} 
                          className="w-full bg-bg-base border border-border-base rounded-xl p-3 text-xs text-text-primary mt-1 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20"
                          placeholder="Tax rate percentage/amount" 
                        />
                      </div>
                      ...
                      {editingId && (
                        <div className="flex items-center pt-2">
                          ...
                          />
                          <label htmlFor="isActive" className="text-xs font-semibold text-text-secondary ml-2 select-none">{t('mill.isActive')}</label>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <>
            ...
            >
              <div>
                ...
                <div className="space-y-4">
                  {auditData.length === 0 && <p className="text-sm text-text-muted">{t('mill.noAudit')}</p>}
                  {auditData.map((audit) => (
                    <div key={audit.auditId} className="p-4 border border-border-base rounded-2xl bg-bg-lighter/30 space-y-3.5">
                      <div className="flex flex-wrap items-center gap-2.5 text-xs text-text-muted">
                        ...
                        <span>&bull;</span>
                        <span>{t('mill.user')}: {audit.userName || audit.userNo || 'System'}</span>
                      </div>
                      ...
                      <div className={`grid ${audit.oldValue && audit.newValue ? 'grid-cols-2' : 'grid-cols-1'} gap-3 text-xs mt-3`}>
                        {audit.oldValue && (
                          <div className="p-3 bg-rose-950/10 text-rose-800 dark:text-rose-200 rounded-xl border border-rose-900/20">
                            <strong className="block mb-1 text-[10px] uppercase tracking-wider text-rose-500">{t('mill.oldValue')}</strong>
                        ...
                        {audit.newValue && (
                          <div className="p-3 bg-emerald-950/10 text-emerald-800 dark:text-emerald-200 rounded-xl border border-emerald-900/20">
                            <strong className="block mb-1 text-[10px] uppercase tracking-wider text-emerald-500">{t('mill.newValue')}</strong>
