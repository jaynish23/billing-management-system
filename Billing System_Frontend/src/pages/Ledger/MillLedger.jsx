...
import { 
  Building, User, Phone, MapPin, Calculator, Calendar, DollarSign, 
  ArrowLeft, Search, Plus, ClipboardList, Filter, FileText
} from 'lucide-react';
...
function MillLedger() {
  ...
  // Client-side search and date range filters
  ...
  const handleExportPdf = async () => {
    ...
    try {
      ...
      const blobData = await pdfService.exportLedger('mill', id, i18n.language);
      
      const blob = new Blob([blobData], { type: 'application/pdf' });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `mill_ledger_${id}_${new Date().toISOString().split('T')[0]}.pdf`);
  ...
  const fetchMillInfo = async () => {
    try {
      const res = await api.get(`/mill/${id}?lang=${i18n.language}`);
      setMillInfo(res.data);
    } catch(err) { 
      console.error(err); 
    }
  };
  ...
  const fetchTransactions = async () => {
    try {
      const res = await api.get(`/LedgerTransaction/mill/${id}?lang=${i18n.language}`);
  ...
  const fetchDukans = async () => {
    try {
      const res = await api.get(`/dukan?page=1&pageSize=1000&lang=${i18n.language}`);
  ...
  const fetchMarkos = async () => {
    try {
      ...
      const filtered = (res.data.data || []).filter(m => m.millNo === parseInt(id));
      setMarkos(filtered);
    } catch(err) { 
      console.error(err); 
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.post('/LedgerTransaction', {
        ...
        millNo: parseInt(id),
        dukanNo: parseInt(formData.dukanNo),
      ...
      fetchTransactions();
      setFormData(prev => ({ ...prev, quantity: '', vigat: '', dukanNo: '', markoNo: '' }));
  ...
  const handleCalculateTax = () => {
    try {
      ...
      const taxInfo = Number(millInfo?.millTaxInfo) || 0;
  ...
  const filteredTransactions = transactions.filter(item => {
    ...
    const itemDate = item.transactionDate ? item.transactionDate.split('T')[0] : '';
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl sm:text-3.5xl font-extrabold text-text-primary tracking-tight flex items-center gap-2.5">
            <span>📒</span>
            <span>{millInfo ? `${millInfo.millName} - Ledger` : 'Loading Ledger...'}</span>
          </h2>
          <p className="text-text-muted text-xs sm:text-sm font-medium mt-1">
            Mill Owner: {millInfo?.ownerName || 'N/A'} | Phone: {millInfo?.ownerPhoneNo || 'N/A'}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glass-card rounded-2xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xl font-black text-text-primary">{millInfo?.millTaxInfo || 0}</p>
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-text-primary truncate max-w-[130px]" title={`${millInfo?.stateName} / ${millInfo?.districtName}`}>
              {millInfo?.stateName || 'N/A'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="glass-card rounded-3xl p-6 border border-border-base">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
          ...
          <div className="lg:col-span-2">
            <label className="text-xs font-semibold text-text-secondary">Party Name (Dukan) *</label>
            <select 
              required 
              name="dukanNo" 
              value={formData.dukanNo} 
              onChange={handleInputChange} 
            ...
            >
              <option value="">-- Select Dukan --</option>
              {dukans.map(d => <option key={d.dukanNo} value={d.dukanNo}>{d.dukanName}</option>)}
            </select>
          </div>
          <div>
            ...
            >
              ...
              {markos.map(m => <option key={m.markoNo} value={m.markoNo}>{m.markoName}</option>)}
            </select>
          </div>
          <div>
      ...
      <div className="glass-card rounded-2xl p-4 border border-border-base flex flex-col md:flex-row md:items-center gap-3">
        <div className="relative flex-1">
          ...
          <input 
            type="text" 
            placeholder="Search transactions by Dukan, Marko or Vigat..." 
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-1.5 text-xs text-text-muted bg-bg-base/40 px-3 py-1.5 rounded-xl border border-border-base/60">
          ...
          {(startDate || endDate || searchTerm) && (
            <button 
              ...
              className="px-3 py-1.5 bg-bg-darker hover:bg-bg-base border border-border-base text-xs font-bold text-text-secondary hover:text-text-primary rounded-lg transition"
            >
              Reset Filters
            </button>
          )}
        ...
        <table className="min-w-full divide-y divide-border-base text-left">
          <thead className="bg-bg-base/40">
            <tr>
              ...
              <th className="px-6 py-3.5 text-xs font-bold text-text-muted uppercase tracking-wider">Party Name (Dukan)</th>
              <th className="px-6 py-3.5 text-xs font-bold text-text-muted uppercase tracking-wider">Marko</th>
              <th className="px-6 py-3.5 text-xs font-bold text-text-muted tracking-wider text-right uppercase">Quantity</th>
          ...
          <tbody className="divide-y divide-border-base bg-transparent text-sm">
            {filteredTransactions.map((item) => (
              <tr key={item.transactionId} className="hover:bg-bg-darker/30 transition-colors">
                <td className="px-6 py-4 whitespace-nowrap text-text-muted font-semibold">
                  {new Date(item.transactionDate).toLocaleDateString()}
                </td>
                <td className="px-6 py-4 whitespace-nowrap font-bold text-blue-600 dark:text-blue-400 tracking-tight">{item.dukanName}</td>
                <td className="px-6 py-4 whitespace-nowrap text-text-secondary font-medium">{item.markoName}</td>
                <td className="px-6 py-4 whitespace-nowrap text-right font-mono text-text-primary font-bold">{item.quantity}</td>
                <td className="px-6 py-4 text-text-muted break-words">{item.vigat}</td>
              </tr>
            ))}
          ...
          {filteredTransactions.length > 0 && (
            <tfoot className="bg-bg-base/40 font-semibold border-t border-border-base">
              <tr>
                ...
                <td className="px-6 py-4 text-right font-mono text-text-primary text-base font-black">{totalQuantity}</td>
                <td></td>
              </tr>
            </tfoot>
          )}
      ...
      {/* Mobile Card List View of Transactions */}
      <div className="grid grid-cols-1 gap-4 md:hidden">
        {filteredTransactions.map((item) => (
          <div key={item.transactionId} className="glass-card rounded-2xl p-4 border border-border-base space-y-3">
            <div className="flex justify-between items-start">
              <div>
                <span className="text-[10px] font-bold text-text-muted uppercase tracking-wider">
                  {new Date(item.transactionDate).toLocaleDateString()}
                </span>
                <h4 className="font-bold text-blue-600 dark:text-blue-400 tracking-tight mt-0.5">{item.dukanName}</h4>
              </div>
              <div className="text-right">
                <span className="text-xs font-bold text-text-muted">Qty</span>
                <p className="font-mono text-text-primary font-black text-sm">{item.quantity}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border-base/60 text-xs">
              <div>
                ...
                <p className="text-text-secondary font-semibold">{item.markoName}</p>
              </div>
              <div>
                <p className="text-[10px] font-bold text-text-muted uppercase tracking-wide">Remarks (Vigat)</p>
                <p className="text-text-muted leading-normal">{item.vigat || 'N/A'}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      {/* Tax Calculation Module at the Bottom */}
      {filteredTransactions.length > 0 && (
        <div className="glass-card rounded-3xl p-6 border border-border-base mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="space-y-1 text-center sm:text-left">
            ...
            <p className="text-xs text-text-muted font-medium">
              Total Qty: <strong className="text-text-primary">{totalQuantity}</strong> &times; Mill Tax: <strong className="text-text-primary">{millInfo?.millTaxInfo || 0}</strong>
            </p>
          </div>
          <div className="flex items-center gap-4 self-stretch sm:self-auto justify-between sm:justify-end">
            <button 
              onClick={handleCalculateTax} 
              className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold shadow-lg shadow-blue-500/15 transition cursor-pointer"
            >
              Calculate Tax
            </button>
            {taxResult !== null && (
              <motion.div 
                ...
                className="bg-blue-500/10 border border-blue-500/20 text-blue-500 px-5 py-2 rounded-xl font-mono font-black text-lg shadow-xl"
              >
