...
import { 
  Store, User, Phone, MapPin, Search, ChevronRight, ArrowLeft 
} from 'lucide-react';
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl sm:text-3.5xl font-extrabold text-text-primary tracking-tight flex items-center gap-2.5">
            <span>🏪</span>
            <span>{t('dukanList') || 'Select Dukan Ledger'}</span>
          </h2>
          <p className="text-text-muted text-xs sm:text-sm font-medium mt-1">
            Choose a Dukan from the listings below to view transactions, calculate taxes, and manage ledger entries.
          </p>
        </div>
        <button 
      ...
      <div className="glass-card rounded-2xl p-4 border border-border-base flex flex-col md:flex-row md:items-center gap-3">
        <div className="relative flex-1">
          ...
          <input 
            type="text" 
            placeholder="Search dukan by name, owner, or contact..." 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredDukans.map((item) => (
          ...
          >
            ...
            <button 
              onClick={() => navigate(`/dukan-ledger/${item.dukanNo}`)}
        ...
        {filteredDukans.length === 0 && (
          <div className="col-span-full py-12 text-center text-text-muted font-medium bg-bg-darker/10 border border-border-base rounded-3xl">
            No Dukan records found matching filters.
          </div>
        )}
      </div>
